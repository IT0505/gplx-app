...
export default function AnswerSection({ answer, setAnswered, listAnswered }) {
  ...
  const handleAnswerVisual = (index) => {
    if (index == listAnswered.index) { // check if current question is answered
      if (listAnswered.isCorrect) { // check if answer correct or not
        return styles.true
      }
      else
        return styles.false
    ...
    <div className={styles.answerSection}>
      ...
      <div className={styles.ansList}>
        {answer.list.map((data, index) => (
          <div key={index} className={`${styles.ansWrap} ${handleAnswerVisual(index)}`}
            onClick={() => {
              if (!listAnswered.isAnswered)
      ...
      <div className={styles.trueAns}>Câu trả lời đúng: {handleAnswerIndex(answer.true_ans)}</div>
      }
    </div>
  )
}