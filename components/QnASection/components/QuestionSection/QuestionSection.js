...
export default function QuestionSection({ question, index, status, listAnswered }) {
  ...
  const calculateResult = () => {
    ...
    for (let i = 0; i < listAnswered.length; i++) {
      if (listAnswered[i].isAnswered)
        if (listAnswered[i].isCorrect)
          score++
    }
  ...
  const displayTimer = () => {
    return `00:${timeLeft.minutes < 10 ? '0' + timeLeft.minutes : timeLeft.minutes}:${timeLeft.seconds < 10 ? '0' + timeLeft.seconds : timeLeft.seconds}`
  }
  ...
  return (
    <div className={styles.questionSection}>
      <div className={styles.titleWrap}>
        ...
        {result.examDone && <span className={styles.result}>{result.content + result.score + '/' + listAnswered.length}</span>}
        {status.isExam && <span className={styles.timer}>{displayTimer()}</span>}
      </div>
